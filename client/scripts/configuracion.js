    ...
    try {
        ...
        if (response.ok) {
            ...
            if (configs.length > 0) {
                ...
                document.getElementById('toleranceMin').value = config.toleranceMin || 0;
            }
        } else {
...
function renderUsersTable() {
    ...
    if (usersData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-center">No hay usuarios disponibles</td></tr>';
        return;
    }
...
function renderNetworkRulesTable() {
    ...
    if (!Array.isArray(networkRules) || networkRules.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center">No hay reglas de red disponibles</td></tr>';
        return;
    }
    ...
    networkRules.forEach((rule) => {
        ...
        row.innerHTML = `
            ...
            <td>${rule.description || 'Sin descripción'}</td>
            <td>
...
async function updateNetworkRule(ruleId, ruleData) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const errorData = await response.json();
            showAlert(errorData.message || 'Error al actualizar regla de red', 'error');
        }
    } catch (error) {
...
function openUserModal(isEdit = false, userId = null) {
    ...
    if (isEdit && userId) {
        ...
        if (user) {
            ...
            document.getElementById('userModality').value = user.modality || 'PRESENTIAL';
...
function openNetworkModal(isEdit = false, ruleId = null) {
    ...
    modalTitle.textContent = isEdit ? 'Editar Regla de Red' : 'Agregar Regla de Red';
    ...
    if (isEdit && ruleId) {
        if (Array.isArray(networkRules)) {
            ...
            if (rule) {
                ...
                document.getElementById('networkIp').value = rule.ipStart || '';
                document.getElementById('networkDescription').value = rule.description || '';
                document.getElementById('networkStatus').value = rule.status || 'ACTIVE';
            } else {
...
window.deleteNetworkRule = function(ruleId) {
    showConfirmModal('¿Está seguro de que desea eliminar esta regla de red?', () => {
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('networkForm').addEventListener('submit', async (e) => {
        ...
        const ruleData = {
            name: formData.get('name'),
            ipStart: formData.get('ip'),
            ipEnd: formData.get('ip'),
