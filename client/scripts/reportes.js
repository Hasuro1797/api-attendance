...
function validateDateRange(startDate, endDate) {
    ...
    if (start > today) {
        return { valid: false, message: 'La fecha de inicio no puede ser mayor a hoy' };
    }
    ...
    if (diffDays > 365) {
        return { valid: false, message: 'El rango de fechas no puede exceder un año' };
    }
...
async function loadDepartments() {
    ...
    try {
        ...
        if (response.ok && data.users) {
            const departments = [...new Set(data.users.map(user => user.department).filter(dep => dep))];
            
            departmentFilterSelect.innerHTML = '<option value="">Todos los Departamentos</option>';
...
function renderReportTable(records) {
    ...
    records.forEach(item => {
        ...
        const fullName = `${userInfo.name || 'N/A'} ${userInfo.lastName || ''}`.trim();
...
function applyClientSideFilters(data, filters) {
    ...
    if (filters.department) {
        filteredData = filteredData.filter(item => 
            item.user && item.user.department === filters.department
        );
    }
    ...
    if (filters.search) {
        ...
        filteredData = filteredData.filter(item => {
            const userInfo = item.user || {};
            const fullName = `${userInfo.name || ''} ${userInfo.lastName || ''}`.toLowerCase();
 ...
 */
async function fetchAttendanceReports(filters = {}, page = 1, limit = itemsPerPage) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (response.status === 401) {
                ...
                console.error('Error al cargar reportes:', data.message || response.statusText);
...
function exportToCSV() {
    ...
    const headers = ['Fecha', 'Usuario', 'Email', 'Departamento', 'Estado', 'Hora Entrada', 'Hora Salida', 'Horas Trabajadas', 'Modalidad'];
    const csvContent = [
        headers.join(','),
        ...filteredData.map(item => {
            ...
            const status = item.status === 'PRESENT' ? 'Presente' : 
                          item.status === 'LATE' ? 'Tarde' : 'Ausente';
    ...
    const dateRange = currentFilters.startDate || currentFilters.endDate ? 
...
function exportToPDF() {
    ...
    const tableData = filteredData.map(item => {
        ...
        return [fecha, fullName, userInfo.email || 'N/A', status, entrada, salida, horas, modality];
    });

    // Crear tabla
    doc.autoTable({
        head: [['Fecha', 'Usuario', 'Email', 'Estado', 'Entrada', 'Salida', 'Horas', 'Modalidad']],
        body: tableData,
        startY: 50,
        styles: { fontSize: 8 },
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Enter' && (e.target === searchUserInput || e.target === startDateInput || e.target === endDateInput)) {
            e.preventDefault();
            applyFiltersAndFetch();
        }
    });
