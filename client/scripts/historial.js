...
async function getProfileForHistorial() {
  ...
  try {
    ...
    console.error(
      'Error al obtener información del usuario para historial:',
      error,
    );
    return null;
  }
...
async function fetchAttendanceHistory(
  ...
  if (filterDaysAgoInput && filterDaysAgoInput.value !== '') {
    ...
    if (!isNaN(parsed) && parsed >= 1) {
      ...
      console.error(
        'El valor para "Días atrás" no es válido (debe ser un número >= 1).',
      );
      if (tableStatus) {
        tableStatus.textContent =
          'Error: "Días atrás" debe ser un número mayor o igual a 1.';
  ...
  if (activeFilterCount > 1) {
    console.error(
      'Error de filtros: No se puede usar más de un filtro de fecha a la vez (Días atrás, Semana actual o Día actual).',
    );
    if (tableStatus) {
      tableStatus.textContent =
        'Error: Por favor, seleccione solo UN tipo de filtro de fecha (Días atrás, Semana actual, o Día actual).';
      tableStatus.style.display = 'block';
    }
    renderHistorialTable([]);
    updatePaginationButtons();
  ...
  if (daysAgoValue !== null) {
    // Usar daysAgoValue que ya está validado
  ...
  try {
    const response = await fetch(
      `/api/attendance/user?${queryParams.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        console.error(
          'No autorizado: Token expirado o inválido. Redirigiendo a login.',
        );
        removeToken();
        window.location.href = '/login.html';
      } else {
        console.error(
          'Error al cargar el historial de asistencia:',
...
function applyFiltersAndFetch() {
  ...
  const filters = {
    ...
    currentWeek: filterCurrentWeekCheckbox
      ? filterCurrentWeekCheckbox.checked
      : false,
    currentDay: filterCurrentDayCheckbox
      ? filterCurrentDayCheckbox.checked
      : false,
  };
    ...
    filterDaysAgoInput.addEventListener('input', () => {
      ...
      if (filterDaysAgoInput.value !== '') {
        if (filterCurrentWeekCheckbox)
          filterCurrentWeekCheckbox.checked = false;
